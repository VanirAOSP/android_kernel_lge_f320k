...
void isdbt_hw_deinit(void)
{
	...
	PRINTF(0, "[FC8150]isdbt_hw_deinit \n");
...
int isdbt_init(void)
{
	...
	if(res < 0)
	{
		...
		if (lge_get_board_revno() >= HW_REV_D  && (is_g2_kddi == 0) ) {
			...
			PRINTF(hInit, "[1seg] A1-DCM rev.D or later version: %d\n",use_pm8941_xo_a2_192000);
		}
		else {
			use_pm8941_xo_a2_192000 = 0;
			PRINTF(hInit, "[1seg] A1-DCM rev.C : %d\n",use_pm8941_xo_a2_192000);
		}
#endif /* USE_PM8941_XO_A2 */
		...
		PRINTF(hInit, "isdbt host interface select fail!\n");
			...
			if (use_pm8941_xo_a2_192000 == 1) {
				if ( !IS_ERR_OR_NULL(clk) )
				{
					...
					ret = clk_prepare_enable(clk);
					if (ret) {
						PRINTF(0,"[1seg] isdbt_init enable clock error!!!\n");
						return -1;
					}
				}
			}
