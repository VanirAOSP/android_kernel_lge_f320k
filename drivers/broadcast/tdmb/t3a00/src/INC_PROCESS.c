...
void INC_RESET_MPI(INC_UINT8 ucI2CID)
{
	...
	/* T3A00 needs 25msec delay for Same Ensemble */
...
void INC_UPLOAD_MODE(INC_UINT8 ucI2CID)
{
	...
	if(m_ucCommandMode != INC_EBI_CTRL){
		...
		// T3A00 (if TSIF, SPI OFF - because PIN Share) 
...
INC_UINT8 INC_PLL_SET(INC_UINT8 ucI2CID) 
{
	...
	case INPUT_CLOCK_24576KHZ:
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x00, 0x0000);	// CLK_SEL OFF (T3A00)
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x01, 0x0001);	// DSMPD (T3A00)
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x02, 0x000F);	// FOUT1PD, FOUT2PD, FOUTVCOPD (T3A00)
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x03, 0x0000);	
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x04, 0x0000);	
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x10, 0x0162);   // ASJ 20130327
	break;
	case INPUT_CLOCK_19200KHZ:
		// Fref = 6.4 MHz, Fvco = 393.216 MHz, Frac. Division Ratio = 0.439999998
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x00, 0x0000);	// CLK_SEL OFF (T3A00)
		...
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x00, 0x0001);	// CLK_SEL ON (T3A00)
		//INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x10, 0x01E2);   // ASJ 20130327
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x10, 0x0422);   // ASJ 20130327
	break;
	default : break;
	}
...
INC_UINT8 INC_INIT(INC_UINT8 ucI2CID)
{
	...
	//INC_CMD_WRITE(ucI2CID, APB_PHY_BASE+ 0xEB, 0x003F);

    INC_CMD_WRITE(ucI2CID, APB_PHY_BASE+ 0x40, 0xF05C);	// enable->0xF05C   disable->0xD05C
	INC_CMD_WRITE(ucI2CID, APB_PHY_BASE+ 0x8C, 0x0183); // 0x0183
...
INC_UINT8 INC_SYNCDETECTOR(INC_UINT8 ucI2CID, INC_UINT32 ulFreq, INC_UINT8 ucScanMode)
{
	...
	while(1) {
		...
		//INC_MSG_PRINTF(1, "\r\n[%d][%d] [0x%.4X][0x%.4X][0x%.4X][0x%.4X]",ulFreq,
			//wOperState, awData[0], awData[1], awData[2], awData[3]);
...
//#define IF_LEVEL_TEST  /* RF Level Debugging */
...
INC_UINT8 INC_ENSEMBLE_SCAN(INC_UINT8 ucI2CID, INC_UINT32 ulFreq)
{
	...
	//printk("[TDMB_T3A00] INC_ENSEMBLE_SCAN (%d)\n", ulFreq);
