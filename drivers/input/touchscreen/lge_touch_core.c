...
#if defined(A1_only)
		ts->gd_ctrl.init_data.mask = GHOST_LONG_PRESS | GHOST_FIRST_IRQ | GHOST_PRESSURE | GHOST_TA_DEBOUNCE;
#else
...
#ifdef CUST_G2_TOUCH
		f54_window_crack = 0;
		if(!f54_window_crack_check_mode || !ts_suspend)
#endif
		{
			if (ts->pdata->role->operation_mode)
				disable_irq(ts->client->irq);
...
#ifdef CUST_G2_TOUCH
	if((lge_get_boot_mode() == LGE_BOOT_MODE_NORMAL) && (atomic_read(&touch_test_dev->device_init) == 1)) {
		queue_delayed_work(touch_wq, &ts->work_f54,
				msecs_to_jiffies(10));
	}
	else
