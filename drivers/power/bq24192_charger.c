...
#define ICHG_MASK 		(BIT(5)|BIT(4)|BIT(3)|BIT(2)|BIT(1)|BIT(0))
...
#define CONFIG_LGE_BATT_REMOVE_SCENARIO // for testing purposes!
...
{
	pr_info("req_chg_timeout set_h = 5hrs\n");
...
#if defined(CONFIG_MACH_MSM8974_Z_KR) || defined(CONFIG_MACH_MSM8974_Z_US)
...
static void bq24192_irq_worker(struct work_struct *work)
{
	...
	if (!bq24192_get_prop_batt_present(chip)) {
		...
		pr_info("battery removed! status(%d-%d-%d)\n",
...
static int bq24192_get_prop_batt_current_now(struct bq24192_chip *chip)
{
	...
	if(rev == HW_REV_A || rev == HW_REV_D)
		return DEFAULT_CURRENT;
#endif
	chip->cn_psy = power_supply_get_by_name("cn");
	if (!chip->cn_psy) {		
		pr_err("cn supply not found, report default current_now\n");
		return DEFAULT_CURRENT;
	} else {
...
#ifdef CONFIG_LGE_THERMALE_CHG_CONTROL
#ifdef CONFIG_MACH_MSM8974_Z_KR // test code!
	if (!(the_chip->ac_online &&
			bq24192_is_charger_present(the_chip))) {
		pr_err("ac_online %d\n", the_chip->ac_online);
		return 0;
	}
	if (bq24192_thermal_mitigation < IBAT_MIN_MA) {
		pr_info("adjust decrease i_limit!\n");
		if (bq24192_thermal_mitigation == 150)
			bq24192_set_input_i_limit(the_chip, 1500);
		else if (bq24192_thermal_mitigation == 120)
			bq24192_set_input_i_limit(the_chip, 1200);
		else if (bq24192_thermal_mitigation == 90)
			bq24192_set_input_i_limit(the_chip, 900);
		return 0;
       }
#endif
...
#ifdef CONFIG_MACH_MSM8974_Z_KR // hg.park@lge.com, for testing purposes!
	if(factory_mode)
		ret = bq24192_set_input_vin_limit(chip, 4440);
	 else
#endif
...
#if defined(CONFIG_MACH_MSM8974_Z_KR) || defined(CONFIG_MACH_MSM8974_Z_US)
		if(lge_get_board_revno() == HW_REV_D)
			bq24192_force_disable_batfet(chip,1);
#endif
	}
