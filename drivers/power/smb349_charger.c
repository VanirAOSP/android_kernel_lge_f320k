	...
	if (wake_lock_active(&smb349_chg->chg_wake_lock)) {
		if( !smb349_is_charger_present(client) ) {
			...
			if (dwc3_usb_present.intval == 1 ){
				...
				ret = smb349_pmic_usb_override(false);
				if (ret)
					pr_err("failed to smb349_pmic_usb_override(false)\n");
...
#ifdef CONFIG_LGE_CHARGER_TEMP_SCENARIO
#define FACTORY_LOWEST_CHG_CURRENT     500
...
smb349_set_thermal_chg_current_set(const char *val, struct kernel_param *kp)
{
	...
	if (is_factory_cable_130k() &&
	       smb349_thermal_mitigation < FACTORY_LOWEST_CHG_CURRENT) {
	       pr_err("factory 130k cable, remain lowest charging current to 500mA(%d)\n",
	               smb349_thermal_mitigation);
	       the_smb349_chg->chg_current_te = FACTORY_LOWEST_CHG_CURRENT;
	} else {
...
static void smb349_irq_worker(struct work_struct *work)
{
	...
	if ( (smb349_chg->usb_present ^ usb_present) ||
		...
		ret = smb349_pmic_usb_override(usb_present);
		if (ret) {
			pr_err("failed to smb349_pmic_usb_override usb_present:%d, ret:%d",
					usb_present, ret);
		}
#ifdef CONFIG_LGE_CHARGER_TEMP_SCENARIO
...
static int smb349_chg_timeout_set(struct smb349_struct *smb349_chg)
{
	...
	/* Complete timeout 764m, pre-charge timeout 48m */
	ret = smb349_masked_write(smb349_chg->client, STAT_TIMER_REG,
				COMPETE_CHG_TIMEOUT_BIT | PRE_CHG_TIMEOUT_BIT, 0x4);
	if (ret) {
...
static int smb349_version_check(struct smb349_struct *smb349_chg)
{
...
#if SMB349_BOOSTBACK_WORKAROUND
	if ( !is_factory_cable() ||
			(is_factory_cable_130k() && smb349_chg->is_bb_work_case) )
#else
	if (!is_factory_cable())
#endif
	{
		/* set AICL detect threshold to 4.5V */
		ret = smb349_set_optichg_det_thr(smb349_chg, OPTICHG_DET_THR_BIT);
		if (ret) {
			pr_err("Failed to set AICL threshold=%d\n", ret);
			return ret;
		} else {
			pr_info("AICL detect threshold 4.5V setted\n");
		}
		...
		if (smb349_chg->is_bb_work_case) {
			...
			if (ret) {
				...
				pr_info("Applied boostback workaround\n");
			}
		}
#endif
	} else {
...
bool external_smb349_is_charger_present(void)
{
	...
	/* uses under voltage status bit to detect cable plug or unplug */
	ret = smb349_read_reg(the_smb349_chg->client, IRQ_E_REG, &irq_status_e);
	if (ret) {
		pr_err("Failed to read IRQ_STATUS_F_REG rc=%d\n", ret);
		return false;
	}
...
#if SMB349_BOOSTBACK_WORKAROUND
	smb349_chg->is_bb_work_case = smb349_version_check(smb349_chg);
#else
...
static void smb349_shutdown(struct i2c_client *client)
{
	 ...
	 * when using low voltage adapter */
		ret = smb349_pmic_usb_override(0);
		if (ret) {
			pr_err("failed to smb349_pmic_usb_override ret : %d\n", ret);
		}
