...
static void charging_information(struct work_struct *work)
{
	...
	int maxim_volt=0;	int maxim_soc=0;
	...
	maxim_volt = get_prop_battery_voltage_now(qpnp_chg)/1000;
	...
	printk(KERN_INFO "[C] USB_IRQ , DC_IRQ , VIN_MIN , VBAT_MAX , VBAT_LOW , Maxim_Volt ,\
	Maxim_SOC , IUSB_MAX , IBAT_MAX , IBAT_TERM , IBAT_NOW , FET_IRQ ,\
	FET_FORCE , BAT_FET , BAT_TEMP , XO_THERM , CHG_STS , Cable_info ,\
	Pseudo_Mode , BAT_PRNC\n");
	printk(KERN_INFO "[I] %d , %d , %d , %d , %d , %d , %d , %d , %d , %d , %d ,\
	0x%x , 0x%x , 0x%x , 0x%x , %d , %d, 0x%x , %d(14) , %d , %d\n",
		usb_in,dc_in,vin,vbat,vdet,maxim_volt,maxim_soc,iusb,ibatm,ibat_term,
