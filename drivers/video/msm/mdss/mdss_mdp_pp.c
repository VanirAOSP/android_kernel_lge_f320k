	...
	if ((pipe->flags & MDP_OVERLAY_PP_CFG_EN) &&
		...
		if (pipe->play_cnt == 0) {
			...
			mdss_mdp_csc_setup(MDSS_MDP_BLOCK_SSPP, pipe->num, 1,
					   MDSS_MDP_CSC_YUV2RGB);
			pr_err("not defined featuring mdss_mdp_csc_setup_data");
#else
			pr_err("pp_vig_pipe_setup_else_dmb_status :%d", dmb_status);
			if(dmb_status == 1) {
				mdss_mdp_csc_setup_data(MDSS_MDP_BLOCK_SSPP, pipe->num, 1, &dmb_csc_convert);
				pr_err("mdss_mdp_csc_setup_data & dmb_status valud : %d", dmb_status);
			} else {
				mdss_mdp_csc_setup(MDSS_MDP_BLOCK_SSPP, pipe->num, 1, MDSS_MDP_CSC_YUV2RGB);
				pr_err("mdss_mdp_csc_setup & dmb_status value : %d", dmb_status);
			}
#endif /* LGE_BROADCAST */
		}
	}
