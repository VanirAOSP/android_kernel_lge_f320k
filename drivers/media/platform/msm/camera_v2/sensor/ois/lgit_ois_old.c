...
#define LAST_UPDATE  "13-07-05, 0x0204B_Q"
...
//	unsigned long	UlLpfCof ;
	...
	if( (StCalDat.UsAdjCompF == 0x0000 ) || (StCalDat.UsAdjCompF & ( EXE_HXADJ - EXE_END )) ){
	...
	if( (StCalDat.UsAdjCompF == 0x0000 ) || (StCalDat.UsAdjCompF & ( EXE_HYADJ - EXE_END )) ){
	...
	if( (StCalDat.UsAdjCompF == 0x0000 ) || (StCalDat.UsAdjCompF & ( EXE_LXADJ - EXE_END )) ){
	...
	if( (StCalDat.UsAdjCompF == 0x0000 ) || (StCalDat.UsAdjCompF & ( EXE_LYADJ - EXE_END )) ){
...
int32_t lgit_ois_move_lens(int16_t offset_x, int16_t offset_y)
{
	...
	// perform move
	RamWriteA(HXSEPT1, offset_x & 0xFFFF);	
	RamWriteA(HYSEPT1, offset_y & 0xFFFF);
	...
	do
	{
		...
		RamReadA(HXIN, &hallx);	
		RamReadA(HYIN, &hally);
	} while (i++ < LENS_MOVE_POLLING_LIMIT && 
			(abs(hallx) > LENS_MOVE_THRESHOLD*HALL_SCALE_FACTOR || 
			 abs(hally) > LENS_MOVE_THRESHOLD*HALL_SCALE_FACTOR ));


	CDBG("%s : (%x) hall %x, %x \n",__func__, i, hallx, hally);

	if (abs(hallx) > LENS_MOVE_THRESHOLD*HALL_SCALE_FACTOR || 
		abs(hally) > LENS_MOVE_THRESHOLD*HALL_SCALE_FACTOR ) { rc = OIS_FAIL; }
