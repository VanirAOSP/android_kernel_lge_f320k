...
static struct usb_endpoint_descriptor charge_only_ss_in_ep_desc = {
...
static struct usb_ss_ep_comp_descriptor charge_only_ss_in_ep_comp_desc = {
	.bLength =              sizeof charge_only_ss_in_ep_comp_desc,
...
static struct usb_descriptor_header *charge_only_ss_descriptors[] = {
	...
	(struct usb_descriptor_header *)&charge_only_ss_in_ep_desc,
	(struct usb_descriptor_header *)&charge_only_ss_in_ep_comp_desc,
	NULL,
};
...
charge_only_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (gadget_is_superspeed(c->cdev->gadget)) {
		charge_only_ss_in_ep_desc.bEndpointAddress =
			charge_only_fs_in_ep_desc.bEndpointAddress;
		...
		unsigned intf, unsigned alt)
{
	...
	if (dev->in_ep != NULL) {
		...
		ret = config_ep_by_speed(cdev->gadget, f, dev->in_ep);
		if (ret) {
			dev->in_ep->desc = NULL;
